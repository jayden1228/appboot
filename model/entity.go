...
func BuildReq(formatList []string, dir string, templatePath string) ([]*EntityReq, error) {
	...
	for idx, table := range tables {
		...
		req.EntityPkg = generator.EntityPackageName
...
func CreateDBEntity(dir string, templatePath string) error {
	...
	if ! (gfile.Basename(dir) == generator.EntityPackageName) {
		dir = gfile.Join(dir, generator.EntityPackageName)
	}
...
func GenerateDbErr(dir string, templatePath string) error {
	...
	errPath := gfile.Join(dir, generator.DbErrFileName)
	...
	if err := tpl.Execute(content, tableInfo{PackageName: generator.EntityPackageName}); err != nil {
		return err
	}
